...
#
# CAPsim
# Plot Module for Multi-Scenario Results
# 
...
def plot_multi_data(export_path, n_scenarios, plots, plotter_start_year, plotter_end_year):
    ...
    maximum = 0
    for plot, (scenario_id, scenario_name, set_years, pp, pa, pc, abs, total, target_year) in enumerate(plots):
        maximum = max(maximum, max(pp), max(pa), max(pc), max(abs), max(total))
